...
const createTaskTool = createTool({
  ...
  execute: async ({
    ...
    try {
      ...
      import('./pattern-learning.mjs').then(({ analyzeTaskPatterns }) => {
        import('./pattern-storage.mjs').then(({ savePattern }) => {
          Promise.resolve().then(function () { return types; }).then(({ Task }) => {
            const taskObj = {
              id: data.id,
              userId,
              title,
              ...
              tags: tags || [],
              sourceRecordingId: sourceRecordingId || void 0,
    ...
    try {
      ...
      if (status === "completed" || priority || category || tags) {
        import('./pattern-learning.mjs').then(({ analyzeTaskPatterns }) => {
          import('./pattern-storage.mjs').then(({ savePattern }) => {
            Promise.resolve().then(function () { return types; }).then(({ Task }) => {
              getSupabaseClient().from("tasks").select("*").eq("id", taskId).single().then(({ data: updatedTask }) => {
                if (updatedTask) {
                  const taskObj = {
                    id: updatedTask.id,
                    userId,
                    title: updatedTask.title,
                    description: updatedTask.description || void 0,
                    priority: updatedTask.priority,
                    status: updatedTask.status,
                    dueDate: updatedTask.due_date ? new Date(updatedTask.due_date) : void 0,
                    dueTime: updatedTask.due_time || void 0,
                    category: updatedTask.category || void 0,
                    tags: updatedTask.tags || [],
                    createdAt: new Date(updatedTask.created_at),
                    completedAt: updatedTask.completed_at ? new Date(updatedTask.completed_at) : void 0
                  };
                  const patterns = analyzeTaskPatterns(taskObj);
                  patterns.forEach((pattern) => {
                    savePattern(userId, pattern.patternType, pattern.patternData).catch(
...
export { types as a, createTaskTool as c, deleteTaskTool as d, getTasksTool as g, taskTools as t, updateTaskTool as u };
