/**
 * Smart Proactive Agent
 * 
 * Intelligent agent that predicts user needs from ANY conversation.
 * No predefined categories - fully dynamic detection.
 * 
 * Key capabilities:
 * - Explicit detection: User clearly asks "draft me a post"
 * - Implicit prediction: User talks about something interesting → predict they might want a post
 * - Casual analysis: User describes their day → predict they might want a summary/report
 * - Context correlation: Connects current voice to past voices for deeper understanding
 */
...
/**
 * Schema for detected intents and predictions
 */
...
export type SmartAnalysis = z.infer<typeof SmartAnalysisSchema>;

/**
 * Tool for outputting smart analysis results
 */
export const outputSmartAnalysisTool = createTool({
...
/**
 * Generate the smart agent prompt based on user context
 */
export async function generateSmartAgentPrompt(
...
/**
 * Get profession-specific guidance for predictions
 */
function getProfessionSpecificGuidance(profession: string): string {
...
/**
 * Create a smart proactive agent with user context
 */
...
/**
 * Analyze a transcription and predict user needs
 */
...
All text must be in ${languageName}.`;
  ...
  if (analysisCall?.args) {
    try {
      const args = typeof analysisCall.args === "string" 
        ? JSON.parse(analysisCall.args) 
        : analysisCall.args;
      return args as SmartAnalysis;
    } catch {
...
/**
 * Default smart agent for standalone use
 */
